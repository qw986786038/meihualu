...
const EdgeInsets _kCameraZoomCapsulePadding =
    ...
    return ListenableBuilder(
      ...
      builder: (_, _) => PopupMenuButton<CameraPreviewRatio>(
        ...
        itemBuilder: (context) => const [
          ...
          PopupMenuItem(
            value: CameraPreviewRatio.ratio1x1,
            child: Text('1:1'),
          ),
          PopupMenuItem(
            value: CameraPreviewRatio.ratio3x4,
            child: Text('3:4'),
          ),
          PopupMenuItem(
    ...
    return ListenableBuilder(
      ...
      builder: (_, _) => SegmentedButton<CameraxOperationMode>(
        ...
        onSelectionChanged: controller.isRecording
            ? null
            : (selection) =>
                controller.setOperationMode(selection.first),
      ),
    );
  }
}
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Slider(
                        ...
                        onChanged: (v) =>
                            unawaited(controller.setZoomLevel(v)),
                      ),
                    ),
...
class CameraLensSwitchButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      listenable: controller,
      builder: (_, _) {
        final enabled = controller.cameras.length > 1 &&
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (_, _) {
        final c = controller;
        final isVideoMode =
            c.operationMode == CameraxOperationMode.video;
