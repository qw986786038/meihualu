...
func cameraxMergeCaptureWithOverlay(
  inputPath: String,
  overlayPath: String,
  outputPath: String,
  quality: Double
  ...
  let overlayUrl = URL(fileURLWithPath: overlayPath) as CFURL
  guard let source = CGImageSourceCreateWithURL(inputUrl, nil),
    let sourceCg = CGImageSourceCreateImageAtIndex(source, 0, nil),
    let overlaySource = CGImageSourceCreateWithURL(overlayUrl, nil),
    let overlayCg = CGImageSourceCreateImageAtIndex(overlaySource, 0, nil)
  else {
    throw CameraxPhotoError.loadFailed
  }
  try cameraxMergeCaptureWithOverlayCG(
    source: source, sourceCg: sourceCg, overlayCg: overlayCg, outputPath: outputPath, quality: quality)
}

func cameraxMergeCaptureWithOverlayData(
  inputPath: String,
  overlayData: Data,
  outputPath: String,
  quality: Double
  ...
  guard let source = CGImageSourceCreateWithURL(inputUrl, nil),
    let sourceCg = CGImageSourceCreateImageAtIndex(source, 0, nil),
    let overlaySource = CGImageSourceCreateWithData(overlayData as CFData, nil),
    let overlayCg = CGImageSourceCreateImageAtIndex(overlaySource, 0, nil)
  else {
    throw CameraxPhotoError.loadFailed
  }
...
private func cameraxMergeCaptureWithOverlayCG(
  ...
  let ci = CIImage(cgImage: sourceCg)
  let oriented = ci.oriented(forExifOrientation: exifOrientation)
  let context = CIContext(options: [.useSoftwareRenderer: false])
  guard let baseCg = context.createCGImage(oriented, from: oriented.extent) else {
    throw CameraxPhotoError.renderFailed
  }
  ...
  cgContext.draw(baseCg, in: rect)
  cgContext.draw(overlayCg, in: rect)
  guard let merged = cgContext.makeImage() else {
    throw CameraxPhotoError.renderFailed
  }
  try writeJpeg(cgImage: merged, outputPath: outputPath, quality: quality)
}
