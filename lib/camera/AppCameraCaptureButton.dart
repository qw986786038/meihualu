...
class AppCameraCaptureButton extends StatefulWidget {
  ...
  const AppCameraCaptureButton({super.key, required this.state});

  @override
  _AppCameraCaptureButtonState createState() => _AppCameraCaptureButtonState();
}

class _AppCameraCaptureButtonState extends State<AppCameraCaptureButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animationController = AnimationController(vsync: this, duration: _duration, lowerBound: 0.0, upperBound: 0.1)
      ..addListener(() {
        setState(() {});
      });
  }
...
class CameraButtonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    bgPainter.color = Colors.black26;
...
class VideoButtonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isRecording) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(17, 17, size.width - (17 * 2), size.height - (17 * 2)), const Radius.circular(12.0)), bgPainter);
    } else {
      bgPainter.color = Colors.red;
      canvas.drawCircle(center, radius - 8, bgPainter);
    }
