...
enum PersonalSpaceUploadTab { all, video }

enum PersonalSpacePickerMode { upload, batch }

class PersonalSpaceUploadController extends GetxController {
  final RxList<AssetEntity> assets = <AssetEntity>[].obs;
  final RxBool isLoading = true.obs;
  final RxSet<String> selectedIds = <String>{}.obs;
  final Rxn<String> permissionMessage = Rxn<String>();
  final Rx<PersonalSpaceUploadTab> activeTab = PersonalSpaceUploadTab.all.obs;
  ...
  List<AssetEntity> get visibleAssets {
    if (activeTab.value == PersonalSpaceUploadTab.all) return assets;
  ...
  Future<void> loadMoreIfNeeded(int currentIndex) async {
    ...
    if (currentIndex < visibleAssets.length - 9) return;
    _isLoadingMore = true;
    await _loadNextPage();
  ...
  void setActiveTab(PersonalSpaceUploadTab tab) {
    activeTab.value = tab;
  }
  ...
  bool isGroupAllSelected(List<AssetEntity> groupAssets) {
    if (groupAssets.isEmpty) return false;
    return groupAssets.every((asset) => selectedIds.contains(asset.id));
  }
