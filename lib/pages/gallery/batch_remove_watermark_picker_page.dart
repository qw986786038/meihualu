...
import 'package:watermark_camera/pages/gallery/batch_remove_watermark_controller.dart';
import 'package:watermark_camera/router/app_paths.dart';

const Color _kPrimaryBlue = Color(0xFF2F7CF6);

class BatchRemoveWatermarkPickerPage extends StatefulWidget {
  const BatchRemoveWatermarkPickerPage({super.key});

  @override
  State<BatchRemoveWatermarkPickerPage> createState() =>
      _BatchRemoveWatermarkPickerPageState();
}

class _BatchRemoveWatermarkPickerPageState
    extends State<BatchRemoveWatermarkPickerPage> {
  final BatchRemoveWatermarkController controller =
      Get.put(BatchRemoveWatermarkController());

  @override
  void dispose() {
    if (Get.isRegistered<BatchRemoveWatermarkController>()) {
      Get.delete<BatchRemoveWatermarkController>();
    }
    super.dispose();
  }
  ...
  void _onBatchRemove() {
    ...
    if (assets.isEmpty) {
      _showSnack('请先选择本应用水印照片或视频');
      return;
    }
    context.push(AppPaths.batchRemoveWatermarkPreview, extra: assets);
  }
    ...
    return Obx(() {
      ...
      return Padding(
        ...
        child: SizedBox(
          ...
          height: 48,
          child: FilledButton(
            onPressed: count == 0 ? null : _onBatchRemove,
