...
void main() {
  ...
  test('Get.lazyPut without fenix', () async {
    ...
    expect(
      () => Get.find<Controller>(),
      throwsA(isA<GetDependencyNotFound>()),
    );
    Get.resetInstance();
  });
  ...
  group('Get.putAsync', () {
    ...
    test('does not re-run builder when already registered', () async {
      var builds = 0;
      Future<Controller> build() async {
        builds++;
        await Future<void>.delayed(Duration.zero);
        return Controller();
      }

      final first = await Get.putAsync<Controller>(build);
      expect(builds, 1);
      final second = await Get.putAsync<Controller>(build);
      expect(builds, 1);
