...
mixin RxObjectMixin<T> on GetListenable<T> {
  ...
  ///
  /// **Prefer calling this inside a [GetBuilder]/[Obx]/[GetX] `builder`
  /// callback.** When called inside a reactive widget, the subscription is
  /// automatically cancelled when the widget unmounts.
  ///
  /// If called outside a reactive build context (e.g. in `onInit()`), the
  /// [StreamSubscription] is NOT tracked and will never be auto-cancelled —
  /// store the return value and cancel it in `onClose()` yourself to avoid
  /// leaking memory.
  ///
  /// ```dart
  /// // Safe: inside Obx — auto-cancelled on unmount
  /// Obx(() { rx.bindStream(someStream); return Text(rx.value); })
  ///
  /// // Safe: manual management in controller
  /// late StreamSubscription _sub;
  /// void onInit() { _sub = myRx.bindStream(stream); }
  /// void onClose() { _sub.cancel(); super.onClose(); }
  /// ```
  StreamSubscription<T> bindStream(Stream<T> stream) {
    if (!Notifier.instance.hasActiveContext) {
      // Not inside a reactive build — the disposer registered via reportAdd
      // will be silently dropped. Log a warning so the developer knows they
      // need to cancel the returned subscription manually.
      Get.log(
        'WARNING: bindStream() was called outside a reactive widget builder '
        '(Obx/GetX/GetBuilder). The subscription will NOT be automatically '
        'cancelled. Store the returned StreamSubscription and cancel it in '
        'onClose() to avoid memory leaks.',
        isError: true,
      );
    }
...
class Rx<T> extends _RxImpl<T> {
  ...
  dynamic toJson() {
    try {
      ...
      throw GetXException('$T has not method [toJson]');
    }
  }
}
